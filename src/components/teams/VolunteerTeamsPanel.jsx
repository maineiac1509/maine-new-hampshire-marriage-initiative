...
export default function VolunteerTeamsPanel() {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <h2 className="text-lg font-semibold tracking-tight">Volunteer Teams</h2>
      ...
      <TeamFormDialog open={createOpen} onOpenChange={setCreateOpen} onCreated={() => loadData()} />

      <div className="relative">
        <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <Input placeholder="Search teams by name or location…" value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
      </div>

      {loading ? (
        <div className="py-12 text-center text-muted-foreground">Loading Volunteer Teams…</div>
      ) : (
        <>
          {/* Desktop table */}
                  ...
                  return (
                    <tr key={t.id} className="border-b last:border-0 transition-colors hover:bg-muted/40">
                      <td className="px-4 py-3.5">
                        <Link to={`/volunteer-teams/${t.id}`} className="font-medium hover:underline">{t.team_name}</Link>
                        {t.family_name && <p className="text-xs text-muted-foreground">{t.family_name}</p>}
                      </td>
                      <td className="px-4 py-3.5 text-muted-foreground">{ms.length ? ms.map((m) => m.display_name).join(', ') : '—'}</td>
                      <td className="px-4 py-3.5">
                        <StatusBadge variant={t.active === false ? 'neutral' : 'success'}>{t.active === false ? 'Inactive' : 'Active'}</StatusBadge>
                      </td>
                      <td className="px-4 py-3.5 text-muted-foreground">{teamLocation(t) || '—'}</td>
          ...
          <div className="space-y-3 md:hidden">
            {filtered.map((t) => {
              ...
              const chs = championsByTeam[t.id] || [];
              return (
                <Link key={t.id} to={`/volunteer-teams/${t.id}`} className="block rounded-xl border bg-card p-4 shadow-sm transition-colors active:bg-muted/50">
